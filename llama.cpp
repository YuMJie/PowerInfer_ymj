...
//
//enum在c++中是一个关键字，用于定义枚举类型的变量。enum是enumeration的缩写，意为枚举。
//也就是llm_arch是一个枚举类型，它的值是LLM_ARCH_LLAMA,LLM_ARCH_FALCON,LLM_ARCH_BAICHUAN,LLM_ARCH_GPT2,LLM_ARCH_GPTJ,LLM_ARCH_GPTNEOX,LLM_ARCH_MPT,LLM_ARCH_STARCODER,LLM_ARCH_PERSIMMON,LLM_ARCH_REFACT,LLM_ARCH_BLOOM,LLM_ARCH_STABLELM,LLM_ARCH_UNKNOWN
enum llm_arch {
    LLM_ARCH_LLAMA,
    LLM_ARCH_FALCON,
    LLM_ARCH_BAICHUAN,
...
struct llama_hparams {
    ...
    // float sparse_pred_threshold = 0.01 ;
...
struct llama_layer {
    ...
    struct ggml_tensor * gpu_idx_rdma;
    struct ggml_tensor * gpu_bucket_rdma;
};
...
struct llama_model_loader {
    ...
    struct ggml_tensor * create_tensor_for(struct ggml_context * ctx, struct ggml_tensor * meta, ggml_backend_type backend) {
        ...
        // backend = GGML_BACKEND_GPU;//有影响

        tensor->backend = backend; // TODO: ggml_set_backend
        // backend = GGML_BACKEND_GPU;//没影响

        ggml_set_name(tensor, ggml_get_name(meta));
        if (backend != GGML_BACKEND_CPU) {
            ggml_set_no_alloc(ctx, use_mmap);
        }
        // tensor->backend = GGML_BACKEND_GPU;//有影响
        n_created++;
    ...
    struct ggml_tensor * create_tensor(struct ggml_context * ctx, const std::string &name, const std::vector<int64_t> & ne, ggml_backend_type backend) {
        struct ggml_tensor * cur = ggml_get_tensor(ctx_meta, name.c_str());//根据名字找到ctx中对应的指针，最后还是调用create_tensor_for
    ...
    void load_all_data(struct ggml_context * ctx, llama_progress_callback progress_callback, void * progress_callback_user_data, llama_mlock * lmlock) {
        ...
        for (int i = 0; i < gguf_get_n_tensors(ctx_gguf); i++) {
            ...
            //     ffn_in_cpu += ggml_nbytes(cur);
            //     cur->backend = GGML_BACKEND_CPU;
            // }
            // printf("cur->name = %s , backend = %d \n"  , cur->name , cur->backend);
            ...
            switch (cur->backend) {
                ...
                case GGML_BACKEND_GPU_SPLIT:
                    ...
                    ++num_layer_gpu;
                    ggml_cuda_transform_tensor(cur->data, cur); //分配显存
                    if (!use_mmap) { 
                        free(cur->data);
                    }
                    break;
        ...
        printf("ffn_in_cpu = %f MiB, num_layer_gpu = %d, num_layer_cpu = %d\n", ffn_in_cpu/(1024.0)/1024, num_layer_gpu, num_layer_cpu); //8256.500000miB  模型大小14.11 GiB 
...
struct llama_gpu_split_loader {
    ...
    llama_gpu_split_loader(const std::string & fname, bool use_mmap , bool use_rdma) : fname(fname), use_mmap(use_mmap) ,use_rdma(use_rdma) {
        ...
        printf("loading gpu_idx from '%s' - please wait ...\n", fname.c_str());
    ...
    int apply_tensors_to_base_model(llama_model * model) {
        ...
        for (int il = 0; il < n_layers; il++) {
            ...
            ggml_tensor * gpu_idx = idx_loader->get_tensor_meta(il*2); //按顺序
            ...
            if(use_rdma)
            {
                model_layer.gpu_idx_rdma = idx_loader->create_tensor_for(ctx_meta, gpu_idx, GGML_BACKEND_CPU);
                model_layer.gpu_bucket_rdma = idx_loader->create_tensor_for(ctx_meta, gpu_bucket, GGML_BACKEND_GPU);
            }
            else
            {
...
void printf_tensor_size(struct ggml_tensor * cur)
{   
    if(cur == NULL)
    {
...
struct llama_augmentation_model_loader {
    ...
    size_t slice_ffn_mat_to_gpu(llama_layer & layer) {
        ...
        // printf("gpu_idx_size");
        // printf_tensor_size(gpu_idx);
        // printf("gpu_bucket_size");
        // printf_tensor_size(gpu_bucket);
        layer.ffn_gate_gpu = create_striped_mat_to_gpu(layer.ffn_gate, gpu_bucket); //将一些在cpu上的ffn_gate按行列分割后放到gpu上
...
struct buffered_tensor_allocator {
    ...
    ggml_tensor * buffered_alloc(const std::string & name, const llm_tensor tensor_type, const std::vector<int64_t> & ne, const int i_layer) {
#if defined(GGML_USE_CUBLAS)
        // printf("GGML_USE_CUBLAS defined\n");
        ...
        return meta_tensor;
#else   
        // printf("GGML_USE_CUBLAS not defined\n");
        return ml.create_tensor(ctx, name, ne, GGML_BACKEND_CPU);
#endif
    }
    ...
    int flush() {
        ...
        for (int enum_i = TENSOR_OFFLOAD_ATTN; enum_i <= TENSOR_OFFLOAD_OUTPUT; enum_i ++) {
            ...
            for (auto tensor_tup : alloc_queues[level]) {
                ...
                // printf("The tensor %s will be offload in GPU \n", meta_tensor->name);
        ...
        printf("num_offloaded_layers: %d\n", num_offloaded_layers); 
        printf("offloaded_layers: %d\n", offloaded_layers); 
        // ml.done_getting_tensors();  尝试不让检查
        return offloaded_layers;
...
static bool load_gpu_split_from_split_file(llama_model & model, std::string split_path, size_t vram_budget ,bool use_rdma = false) {
    printf("split_path: %s\n", split_path.c_str());
    llama_gpu_split_loader loader(split_path, true,use_rdma); //把idx已经加载到loader中
    return loader.check_vram_allocable(vram_budget) 
        && loader.apply_tensors_to_base_model(&model) == 0;
}

static bool llm_load_gpu_split_with_budget_for_rdma(llama_model_loader & ml, llama_model & model, size_t vram_allocatable_bytes, bool no_cache) {//生成gpuidx文件
...
static bool llm_load_gpu_split_with_budget(llama_model_loader & ml, llama_model & model, size_t vram_allocatable_bytes, bool no_cache) {//生成gpuidx文件
...
static size_t llm_load_gpu_split(llama_model_loader & ml, llama_model & model, bool no_cache, bool no_offload) {
    ...
    if (!no_offload && !llm_load_gpu_split_with_budget_for_rdma(ml, model, vram_budget_bytes, no_cache)) {
        LLAMA_LOG_ERROR("%s: error: failed to generate rdma split, an empty one will be used\n", __func__);
    }
#endif
...
llama_model * sparse_model = new llama_model;
static void llm_load_sparse_model_tensors(
            ...
            case LLM_ARCH_REFACT:
                {
                    ...
                    // output
                     {   
                        //printf("tn(LLM_TENSOR_OUTPUT_NORM, weight)=%s\n", tn(LLM_TENSOR_OUTPUT_NORM, "weight").first.c_str()); tn(LLM_TENSOR_OUTPUT_NORM, weight)=output_norm.weight
                    ...
                    sparse_model->layers.resize(n_layer);

                    for (uint32_t &i = current_layer; i < n_layer; ++i) {
                       auto & layer = model.layers[i];
                       auto & sparse_layer = sparse_model->layers[i];
                        ...
                        sparse_layer.mlp_pre_w1 = create_tensor(tn(LLM_TENSOR_MLP_PRED_FC1, "weight", i), {n_embd, GGML_NE_WILDCARD});
                        sparse_layer.mlp_pre_w2 = create_tensor(tn(LLM_TENSOR_MLP_PRED_FC2, "weight", i), {GGML_NE_WILDCARD, n_ff});
    ...
    printf("start populate `tensors_by_name`\n");
    for (int i = 0; i < ml.n_tensors; ++i) {
        // printf("name=%s\n", ml.get_tensor_name(i));

        struct ggml_tensor * cur = ggml_get_tensor(ctx, ml.get_tensor_name(i));
        // printf("name=%s\n", ml.get_tensor_name(i));

        // printf("ggml_get_tensor(ctx, ml.get_tensor_name(i));");
        // sleep(8);
        // printf("name2=%s \n", ggml_get_name(cur));
        model.tensors_by_name.emplace_back(ggml_get_name(cur), cur);
        sparse_model->tensors_by_name.emplace_back(ggml_get_name(cur), cur);
        // printf("model.tensors_by_name.emplace_back(ggml_get_name(cur), cur);");
        // sleep(8);

    }
    printf("end populate `tensors_by_name`\n");
    // printf("before load_all_data1\n");//618M
    //根据ggml的上下文加载所有的数据，进行调度
    ml.load_all_data(ctx, progress_callback, progress_callback_user_data, use_mlock ? &model.mlock_mmap : NULL);
    // printf("after load_all_data\n");//6592M
...
// static void llm_load_sparse_model_tensors1(
//         llama_model_loader & ml,
//         llama_model & model,
//         const llama_context_params * cparams,
//         int main_gpu,
...
void output_model_value(llama_model & model,int layer_id)
{   
    for(int j =0;j<layer_id;++j)
    {
            auto layer = model.layers[j];
    if(layer.ffn_gate) {
        void * data = layer.ffn_gate->data;
        printf("layer_id:%d, data:%f\n",j,((float *)layer.ffn_gate->data)[0]);
        printf("ffn_gate.ne = %d,%d,%d,%d\n", layer.ffn_gate->ne[0],layer.ffn_gate->ne[1],layer.ffn_gate->ne[2],layer.ffn_gate->ne[3]);
        for(int i=0;i<3;++i)
        {
            printf("ffn_gate_data[%d] = %.15lf\n",i,((double *)layer.ffn_gate->data)[i]);
        
        }
    }
    if(layer.ffn_down && layer.ffn_down->data) {
        printf("ffn_down is not null\n");
        printf("ffn_down.ne = %d,%d,%d,%d\n", layer.ffn_down->ne[0],layer.ffn_down->ne[1],layer.ffn_down->ne[2],layer.ffn_down->ne[3]);
        void * data = layer.ffn_down->data;
        printf("ffn_down layer_id:%d, data:%.15lf\n",j,((double *)layer.ffn_down->data)[0]);
        // printf("size of ((float *)data) =%d" ,malloc_usable_size(data) );
    }
    if(layer.ffn_up && layer.ffn_up->data) {
        printf("ffn_up is not null\n");
        void * data = layer.ffn_up->data;
        printf("ffn_up.ne = %d,%d,%d,%d\n", layer.ffn_up->ne[0],layer.ffn_up->ne[1],layer.ffn_up->ne[2],layer.ffn_up->ne[3]);
        for(int i=0;i<3;++i)
        {
            printf("ffn_up_data[%d] = %.15lf\n",i,((double *)layer.ffn_up->data)[i]);
        
        }
        printf("ffn_up layer_id:%d, data:%f\n",j,((float *)layer.ffn_up->data)[0]);
        // printf("size of ((float *)data) =%d" ,malloc_usable_size(data) );
    }
    }
    ...
    // if(layer.) {
    //     void * data = layer.ffn_up->data;
    //     printf("ffn_dup layer_id:%d, data:%f\n",layer_id,((float *)layer.ffn_down->data)[0]);
    //     // printf("size of ((float *)data) =%d" ,malloc_usable_size(data) );
    // }
    // printf("size of data:%d\n",sizeof(*(float *)data));
}

static bool llama_sparse_model_load(const std::string & fname, llama_model & model, const llama_model_params & params, const llama_context_params * cparams) {
    try {
        // params.n_layer-=10;
        ...
        //use vram budget if sparse inference is enabled
        if (llama_use_sparse_inference(&model)) {
            ...
            // llm_load_sparse_model_tensors1(
            //     ml, model, cparams, params.main_gpu, vram_budget_bytes, params.reset_gpu_index, params.disable_gpu_index,
            //     params.use_mlock, params.progress_callback, params.progress_callback_user_data
            // );
        } else {
...
static bool llama_model_load(const std::string & fname, llama_model & model, const llama_model_params & params, const llama_context_params * cparams) {
    ...
    // output_model_value(model,10);
    // llama_sparse_model_load(fname,*sparse_model,params,cparams);
    return true;
}
...
void output_value (ggml_tensor * cur)
{
    if(cur )
    {
        for(int i=0;i<4;++i)
        {
            printf("name:%s ,data:%.17lf\n",cur->name,((double *)cur->data)[i]);
        }
    }
}
static struct ggml_tensor * llm_build_ffn_sparse(
        struct ggml_context * ctx,
         ...
         struct ggml_tensor * gpu_index,//维度固定，value为一表示在gpu上做运算
         struct ggml_tensor * gpu_bucket,//表示在gpu上做运算的行的下标，有一维度和down_gpu对应
         struct ggml_tensor * gate_gpu,
         struct ggml_tensor * down_gpu,//只包含需要在gpu上做运算的行
    ...
    // printf("llm_build_ffn_sparse_down->backend: %d\n", down->backend);
    ggml_tensor *idx = nullptr;
    ggml_tensor *idx1 = nullptr;
    ggml_tensor *idx_g = nullptr;
    ggml_tensor *cur_c = nullptr;
    ggml_tensor *third = nullptr;

    ggml_tensor * sparse_pre_w1 =sparse_model->layers[il].mlp_pre_w1;
    ggml_tensor * sparse_pre_w2 =sparse_model->layers[il].mlp_pre_w2;
    // ggml_tensor * sparse_pre_w1 =pre_w1;
    // ggml_tensor * sparse_pre_w2 =pre_w2;
    ...
    // prepare sparse idx
    idx1 = ggml_mul_mat(ctx, sparse_pre_w1, pred_inpl);
    // no offlaoad
    cb(idx, "mlp_pre_w1", il);
    idx = ggml_relu(ctx, idx1);
    cb(idx, "relu_pre", il);
    idx = ggml_mul_mat(ctx, sparse_pre_w2, idx);
    ...
    // {   
        // printf("input_cur : ");
        // printf_tensor_size(pred_inpl);
        // printf("pred_inpl : ");
        // printf_tensor_size(pred_inpl);
        // printf("pre_w1 : ");
        // printf_tensor_size(pre_w1);
        // printf("idx1 : ");
        // printf_tensor_size(idx1);
        // printf("pre_w2 : ");
        // printf_tensor_size(pre_w2);
        // printf("idx : ");
        // printf_tensor_size(idx);
        // printf("gpu_index : ");
        // printf_tensor_size(gpu_index);
        // printf("gpu_bucket : ");
        // printf_tensor_size(gpu_bucket);
        // printf("up : ");
        // printf_tensor_size(up);
        // printf("up_gpu : ");
        // printf_tensor_size(up_gpu);
        // printf("gate : ");
        // printf_tensor_size(gate);
        // printf("gate_gpu : ");
        // printf_tensor_size(gate_gpu);        
        // printf("down : ");
        // printf_tensor_size(down);
        // printf("down_t : ");
        // printf_tensor_size(down_t);
        // printf("down_gpu : ");
        // printf_tensor_size(down_gpu);
...
static struct ggml_tensor * llm_build_ffn_sparse_rdma(
        struct ggml_context * ctx,
         ...
         struct ggml_tensor * gpu_index,//维度固定，value为一表示在gpu上做运算
         struct ggml_tensor * gpu_bucket,//表示在gpu上做运算的行的下标，有一维度和down_gpu对应
         struct ggml_tensor * gpu_index_rdma,
         struct ggml_tensor * gpu_bucket_rdma,
         struct ggml_tensor * gate_gpu,
         struct ggml_tensor * down_gpu,//只包含需要在gpu上做运算的行
         struct ggml_tensor * up_gpu,
    ...
    ggml_tensor *cur_c = nullptr;
    ggml_tensor *third = nullptr;
    ggml_tensor *temp_rdma_idx = nullptr;
...
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama() {//进行稀疏推理
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // feed-forward network
            {
                ...
                if (1) {
                    cur = llm_build_ffn_sparse_rdma(ctx0, cur, //改为edma
                        ...
                        // inpSA, // use last input
                        ffn_inp, // as for now, llama's pred use the same input as the ffn
                        model.layers[il].gpu_idx, 
                        model.layers[il].gpu_bucket,
                        model.layers[il].gpu_idx_rdma, 
                        model.layers[il].gpu_bucket_rdma,                       
                        model.layers[il].ffn_gate_gpu, model.layers[il].ffn_down_gpu, model.layers[il].ffn_up_gpu, // TODO: disable gpu offloading as for now
    ...
    struct ggml_cgraph * build_llama_dense() { //在Gpu上进行推理
        struct ggml_cgraph * gf = ggml_new_graph_custom(ctx0, LLAMA_MAX_NODES, false);
...
static int llama_decode_internal(
    ...
    ggml_graph_compute_helper(lctx.work_buffer, gf, n_threads); //计算模型
#endif
    ...
    {   
        ...
        // float * logits = lctx.logits.data();
        // printf ("decode_logits[0] = %f\n", logits[0]);
        // printf ("logits_out[0] = %f\n", logits_out.data()[0]);
        if (batch.logits) {
            ...
            // printf ("before_logits_out[0] = %f\n ,logits_out.size() = %d \n", logits_out.data()[0], (int)logits_out.size());
            logits_out.resize(n_vocab);
            memcpy(logits_out.data(), (float *) ggml_get_data(res) + (n_vocab*(n_tokens - 1)), sizeof(float)*n_vocab);
            // printf ("after_logits_out[0] = %f\n ,logits_out.size() = %d \n", logits_out.data()[0], (int)logits_out.size());

        }
        // printf ("logits_out[0] = %f\n", logits_out.data()[0]); 已经进入到模型
